...
class DietPlanListController extends Controller
{
    ...
    public function edit(FeedDietPlan $plan)
    {
        return view('feeding.diet_plan_edit', array_merge(
            ['plan' => $plan],
            $this->getFormViewData(),
        ));
    }
    ...
    {
        $data = $this->validateEditScreenPayload($request);
        $subtypes = $this->extractSubtypePayload($request);
    ...
    private function validateEditScreenPayload(Request $request): array
    {
        return $request->validate([
            'farmer_id' => ['required', 'exists:farmers,id'],
            'animal_id' => ['nullable', 'exists:animals,id'],
