...
<div class="container-fluid">
    <div class="d-flex flex-wrap align-items-center justify-content-between gap-2 mt-3 mb-3">
        <div>
            ...
            <p class="text-muted mb-0">Track farmers referred by doctors and reward eligibility.</p>
        </div>
    </div>
    ...
    <div class="row g-3 mb-4">
        ...
        <div class="col-md-6 col-xl-3">
            <div class="card border-0 shadow-lg h-100 overflow-hidden" style="background: linear-gradient(135deg, #9b59b6, #6f42c1); border-radius:16px;">
                <div class="card-body text-white p-4 d-flex justify-content-between align-items-start">
                    <div>
                        <p class="mb-2 text-white-50 fw-semibold">Doctor Points</p>
    ...
    <div class="card border-0 shadow-sm">
        <div class="card-header bg-transparent border-0 pt-4 pb-2">
            <h5 class="mb-0">Referred Farmer Entries</h5>
        </div>
                ...
                <table class="table align-middle mb-0">
                    ...
                    <tbody>
                        @forelse($referredFarmers as $index => $farmer)
                            @php
                                $doctor = $farmer->referredByDoctor;
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="fw-semibold">{{ $doctorName !== '' ? $doctorName : '-' }}</div>
                                    <small class="text-muted">{{ optional($doctor)->contact_number ?: '-' }}</small>
                                </td>
                                <td><span class="badge bg-light text-dark">{{ $farmer->doctor_referral_code ?: optional($doctor)->referral_code ?: '-' }}</span></td>
                                <td>
                                ...
                                <td>
                                    @if($farmer->referral_reward_granted_at)
                                        <span class="badge bg-success-subtle text-success">Granted</span>
                                        <div class="small text-muted mt-1">{{ optional($farmer->referral_reward_granted_at)->format('d-m-Y') }}</div>
                                    @else
                        ...
                        @empty
                            <tr>
                                <td colspan="7" class="text-center text-muted py-4">No referred farmer records found.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
