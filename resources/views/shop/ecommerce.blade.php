...
@section('content')
@php
    $activeTab = request('tab', 'add-product');
@endphp
<div class="container-fluid">
        ...
        <div class="col-md-3">
            <div class="card bg-warning-subtle border-0">
                <div class="card-body">
                    ...
                    <h3 class="mb-0">{{ $summary['new_orders'] }}</h3>
                </div>
            </div>
        </div>
        <div class="col-md-3">
            <div class="card bg-info-subtle border-0">
                <div class="card-body">
                    ...
                    <h3 class="mb-0">{{ $summary['in_progress_orders'] }}</h3>
                </div>
            </div>
        </div>
        <div class="col-md-3">
            <div class="card bg-success-subtle border-0">
                <div class="card-body">
                    ...
                    <h3 class="mb-0">{{ $summary['completed_orders'] }}</h3>
                </div>
            </div>
        </div>
    </div>
    ...
    @if($activeTab === 'new-order')
        @include('shop.partials.order_table', ['orders' => $newOrders, 'title' => 'New Orders', 'showActions' => true])
    @endif

    @if($activeTab === 'in-progress')
        @include('shop.partials.order_table', ['orders' => $inProgressOrders, 'title' => 'Orders In Progress', 'showActions' => true])
    @endif

    @if($activeTab === 'completed')
        @include('shop.partials.order_table', ['orders' => $completedOrders, 'title' => 'Completed Orders', 'showActions' => false])
    @endif

    @if($activeTab === 'payment')
        <div class="card">
            <div class="card-header"><h5 class="mb-0">Order Payment</h5></div>
            <div class="card-body">
                <div class="table-responsive">
                        ...
                        <tbody>
                            @forelse($paymentOrders as $order)
                                <tr>
                                    ...
                                    <td>
                                        <span class="badge {{ strtolower((string) $order->payment_status) === 'paid' ? 'bg-success-subtle text-success' : 'bg-warning-subtle text-warning' }}">
                                    ...
                                    <td>
                                        @if(strtolower((string) $order->payment_status) !== 'paid')
                                            <form method="POST" action="{{ route('shop.orders.status', $order) }}" class="d-inline">
                                                @csrf
            ...
            <form method="POST" action="{{ route('shop.store') }}" enctype="multipart/form-data">
                ...
                <div class="modal-body">
                    <div class="row g-3">
                        ...
                        <div class="col-12"><label class="form-label">Features (one per line)</label><textarea name="features" rows="3" class="form-control"></textarea></div>
