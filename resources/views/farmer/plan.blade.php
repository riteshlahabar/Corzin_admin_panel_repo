...
<div class="container-fluid">
    ...
    <div class="d-flex flex-wrap align-items-center justify-content-between gap-2 mb-3">
        <h4 class="mb-0 text-dark">Farmer Plan</h4>
        <small class="text-muted">Create and manage subscription plans for farmers.</small>
    </div>
        ...
        <div class="col-lg-4">
            <div class="card border-0 shadow-sm">
                <div class="card-body">
                    ...
                    <form method="POST" action="{{ route('farmer.plan.store') }}">
                        @csrf
                        <div class="mb-3">
                        ...
                        <div class="form-check mb-3">
                            <input class="form-check-input" type="checkbox" id="isActivePlan" name="is_active" value="1" {{ old('is_active', '1') ? 'checked' : '' }}>
                            <label class="form-check-label" for="isActivePlan">Active Plan</label>
                        </div>
                            ...
                            <tbody>
                                @forelse($plans as $plan)
                                    <tr>
                                        ...
                                        <td class="text-end">
                                            <button type="button" class="btn btn-sm btn-light border" data-bs-toggle="modal" data-bs-target="#editFarmerPlan{{ $plan->id }}">
                                                Edit
                                            </button>
                                        </td>
                                    </tr>
                                @empty
                                    <tr>
                                        <td colspan="7" class="text-center text-muted py-4">No farmer plans found.</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
    ...
    @foreach($plans as $plan)
        <div class="modal fade" id="editFarmerPlan{{ $plan->id }}" tabindex="-1" aria-hidden="true">
            <div class="modal-dialog modal-lg modal-dialog-centered">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title">Edit Farmer Plan</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <form method="POST" action="{{ route('farmer.plan.update', $plan) }}">
                        @csrf
                        @method('PUT')
                            ...
                            <div class="row g-3">
                                ...
                                <div class="col-12">
                                    <div class="form-check">
                                        <input class="form-check-input" type="checkbox" id="farmerPlanActive{{ $plan->id }}" name="is_active" value="1" {{ $plan->is_active ? 'checked' : '' }}>
                                        <label class="form-check-label" for="farmerPlanActive{{ $plan->id }}">Active Plan</label>
                                    </div>
                                </div>
                            </div>
                        </div>
